...
plot_total_ads <- ggplotly(summary_dataset %>%
  ...
  ggtitle(paste("Ads since", format(start_date, "%d.%m.%Y"), "of pages with more than 10 ads")))
...
spend_weekly <- ggplotly(
  time_dataset %>%
    ...
    scale_x_date(date_breaks = "1 months", date_labels = "%d.%m.%y") +
    coord_cartesian(xlim = c(start_date, start_date), expand = TRUE) +
    theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1)) +
    xlab(element_blank()) +
    ylab("CZK thousand") +
    ggtitle("Weekly spending (in thousands CZK) on FB Ads, January 2021-present")
)
