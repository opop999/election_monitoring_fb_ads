...
# TEMPORARY SUBFILTER:
# Filter out entities, which are not current 2021 election candidates (General Pavel, Milion Chvilek)
top_10_spenders <- summary_dataset %>%
  filter(!page_id %in% c(102389958091735, 728495140691300)) %>% 
	# Filtering out individual politicians - these top 10 are among political parties only
	filter(!page_id %in% c(179497582061065,
												 487445514669670,
												 134443820058669,
												 356451014434612,
												 197010357446014,
...
top_10_spenders_individuals <- summary_dataset %>%
  filter(!page_id %in% c(102389958091735, 728495140691300)) %>% 
	# Filtering out political parties - this top 10 is for individual politicians
	filter(page_id %in% c(179497582061065,
												 487445514669670,
												 134443820058669,
												 356451014434612,
...
plot_total_spending <- ggplotly(summary_dataset %>%
  ...
  ggtitle(paste("Spending on Ads (pages with more than 10 ads) since", format(start_date, "%d.%m.%Y"))))
...
```{r}
plot_avg_length <- ggplotly(summary_dataset %>%
  filter(total_ads > 10) %>%
  ggplot(aes(x = avg_words, y = reorder(page_name, avg_words))) +
  ...
  xlab("Words") +
  ylab(element_blank()) +
  ggtitle(paste("Average Ad lenght (in words, pages with more than 10 ads) since", format(start_date, "%d.%m.%Y"))))

plot_avg_length

saveWidget(plot_avg_length, file = paste0(directory, "/plot_avg_length.html"))
...
plot_total_runtime <- ggplotly(summary_dataset %>%
  ...
  xlab("Days") +
  ylab(element_blank()) +
  ggtitle(paste("Average Ad runtime (pages with more than 10 ads) since", format(start_date, "%d.%m.%Y"))))
...
plot_total_impressions <- ggplotly(summary_dataset %>%
  ...
  theme_minimal() +
  xlab("Total Impressions (millions, square root scale)") +
  ylab(element_blank()) +
  labs(fill = "Per Ad impressions (thousands)") +
  ggtitle(paste("Total Ads impressions (pages with more than 10 ads) since", format(start_date, "%d.%m.%Y"))))
...
### **SPENDING OVER TIME (Organizations)**
...
spend_over_time <- ggplotly(
  time_dataset %>%
    ...
    ylab("CZK million") +
  	labs(color = "") +
    ggtitle(paste("Spending (in CZK million) on Ads of 10 biggest spenders since", format(start_date, "%d.%m.%Y")))
)
...
### **SPENDING OVER TIME (Individuals)**

```{r}
spend_over_time_individuals <- ggplotly(
  time_dataset %>%
    filter(page_id %in% top_10_spenders_individuals$page_id) %>%
    mutate(
      ad_creation_time_week = as.Date(cut(ad_creation_time, breaks = "week", start.on.monday = TRUE)) + 4,
      cumulative_spend_million = round((cumulative_spend / 1000000), digits = 3)
    ) %>%
...
spend_over_time_individuals

saveWidget(spend_over_time_individuals, file = paste0(directory, "/plot_spend_over_time_individuals.html"))

```

### **SPENDING WEEKLY (Organizations)**
...
spend_weekly <- ggplotly(
  time_dataset %>%
    ...
    ylab("CZK thousand") +
  	labs(fill = "") +
    ggtitle(paste("Weekly spending (in CZK thousand) on Ads of 10 biggest spenders since", format(start_date, "%d.%m.%Y")))
)
...
### **SPENDING WEEKLY (Individuals)**

```{r}
spend_weekly_individuals <- ggplotly(
  time_dataset %>%
    filter(page_id %in% top_10_spenders_individuals$page_id) %>%
    mutate(
      end_of_week = as.Date(cut(ad_creation_time, breaks = "week", start.on.monday = TRUE)) + 4,
...
spend_weekly_individuals

saveWidget(spend_weekly_individuals, file = paste0(directory, "/plot_spend_weekly_individuals.html"))
...
plot_age <- ggplotly(
  summary_dataset %>%
    ...
    ylab(element_blank()) +
  	labs(fill = "") +
    ggtitle(paste("Proportion of Ads of 10 biggest spenders targeting specific age group since", format(start_date, "%d.%m.%Y")))
)
...
plot_age_individuals

saveWidget(plot_age_individuals, file = paste0(directory, "/plot_age_individuals.html"))
...
plot_region <- ggplotly(
  summary_dataset %>%
    ...
    ylab(element_blank()) +
  	labs(fill = "") +
    ggtitle(paste("Proportion of Ads of 10 biggest spenders targeting specific region since", format(start_date, "%d.%m.%Y")))
)
...
```{r}
plot_region_individuals <- ggplotly(
  summary_dataset %>%
    filter(page_id %in% top_10_spenders_individuals$page_id) %>%
    select(
      page_name,
...
plot_region_individuals

saveWidget(plot_region_individuals, file = paste0(directory, "/plot_region_individuals.html"))
